...
async function startServer() {
  ...
  app.get("/api/auth/google/callback", async (req, res) => {
    ...
    if (queryError) {
      console.error("Google OAuth error from query:", queryError);
      return res.status(400).send(`Error de Google: ${queryError}. Por favor, verifica la configuración de tu proyecto en Google Cloud.`);
    }
    ...
    if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET) {
      return res.status(500).send("Error de configuración: GOOGLE_CLIENT_ID o GOOGLE_CLIENT_SECRET no están configurados en los ajustes de AI Studio.");
    }
    ...
    try {
      ...
      res.status(500).send(`Error en la autenticación: ${error.message}. Asegúrate de que la URL de redirección en Google Cloud sea exactamente: ${redirectUri}`);
    }
  });
